...
(function(){
	...
	var yEnc = function(){
		...
		//
        // > `@param encryptKey [string]` the encryption key to use to make the encoded file unique
		...
		self.encode = function(source) {
			return self.encodeBytes(toBytes(source)); // To seperate the string from the token.
		};
        ...
        // If the encryption key isn't included inside the decoded string, it
		self.decode = function(source) {
        let verifyEncrypt = fromBytes(self.decodeBytes(source)); 
		return verifyEncrypt;
		};
