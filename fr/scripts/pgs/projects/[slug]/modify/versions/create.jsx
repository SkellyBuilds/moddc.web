...
export default function creatingJSX({React, ReactD, glob, DynamicComponentCommunicator}){
...
async function uploadFile() {
    ...
    try {
        const response = await fetch(glob.apiurlP+'projects/versions/cwf?slug='+DynamicComponentCommunicator.params[0], {
            method: 'POST',
            body: formData,
        });

        if (!response.ok) {
            setError(`Server error related to creating a version: ${response.status} ${response.statusText}`);
            return;
        }
        ...
        if(result.ok){
            setInterval(() => {
                DynamicComponentCommunicator.getNSwitchComponent("/projects/"+DynamicComponentCommunicator.params[0]+"/versions/"+result.vId);
            }, 925)
...
<>
    {
         data.ok && dataA != null ?
            ...
            { 
                ...
                <>
                Creating a new version <br />
                You can only upload the file. Everything will be handled
                <br />
                <input type="file" name="file" accept=".jar" required /> <br />
                <button id="submitC" className="text-white bg-red-500" onClick={uploadFile}>Upload & create</button>
                </>
